...
public class UpdateStatusCommand(IVideoRepository videoRepository, ISendEmailCommand sendEmailCommand) : IUpdateStatusCommand
{
    ...
    public async Task<VideoResult> Execute(IFormFile arquivo, string usuario, int id, string caminho, VideoStatus status)
    {
        try
        {
            ...
            var request = Helper.MapRequest(arquivo, usuario, conteudo, caminho);
            request.Id = videoBase.Id;
            videoBase.NomeArquivo = request.NomeArquivo;
            videoBase.Status = status;
            videoBase.Conteudo = request.Conteudo;
            videoBase.Caminho = request.Caminho;
